<template>
  <el-row class="clearfix">
    <el-col :span="24">
      <div class="main">
        <div class="title"><span>院内公告</span></div>
        <div class="list">
          <ul>
            <li v-for="item in list" :key="item.id">
              <router-link  class="ellipsis after" :to="{path:'noticeDetail/' + item.id}">{{item.noticeTitle}}</router-link>
              <span>{{item.noticeCreateTime.substring(0,10)}}</span>
            </li>
          </ul>
        </div>
        <div class="block">
...
export default {
  ...
  async asyncData ({route, router, store}) {
    await store.dispatch('getNoticeList', {current: 1, size: 10})
  },
  created () {
